...
RSS_URL = "https://www.khk.cz/rss.xml"
...
def parse_date(date_str):
    """Parse date string in format 'Fri, 31 Dec 2024 23:59:59 GMT' or 'Wed, 07 May 2025 11:44:51 +0000' to datetime object"""
    try:
        # Try parsing with timezone offset first (e.g., +0000)
        return datetime.strptime(date_str, '%a, %d %b %Y %H:%M:%S %z')
    except ValueError:
        # Fallback to parsing with timezone name (e.g., GMT)
        try:
            return datetime.strptime(date_str, '%a, %d %b %Y %H:%M:%S %Z')
        except ValueError as e:
            logger.error(f"Error parsing date '{date_str}' with both %z and %Z formats: {e}")
            return None
...
def parse_rss_feed(content):
    ...
    if last_processed_date:
        ...
        if last_processed_dt: # Ensure last_processed_dt is not None
            last_processed_dt = last_processed_dt.replace(tzinfo=None) # Make naive
            logger.info(f"Found last processed date: {last_processed_dt.strftime('%Y-%m-%d')}")
        else:
            logger.warning(f"Could not parse last processed date: {last_processed_date}")
            # last_processed_dt remains None
    else:
    ...
    for item in items:
        ...
        pub_date_dt = pub_date_dt.replace(tzinfo=None) # Make naive
...
def extract_item_data(item):
    ...
    # Description extraction from CDATA
    description_html_content = item.description.get_text(strip=False) # Get raw CDATA
    soup_description = BeautifulSoup(description_html_content, 'html.parser')

    # Extract text from <p> tags for the main description
    description_paragraphs = soup_description.find_all('p')
    if description_paragraphs:
        description = "\\n".join([p.get_text(strip=True) for p in description_paragraphs])
    else:
        # Fallback: if no <p> tags, get all text from description CDATA
        description = soup_description.get_text(strip=True)

    # Image URL extraction from CDATA
    image_url = None
    # Look for an <a> tag with 'data-sub-html' attribute, as these seem to wrap/link to images
    image_anchor = soup_description.find('a', attrs={'data-sub-html': True})
    if image_anchor and image_anchor.has_attr('href'):
        # Use urljoin to handle both absolute and relative URLs correctly against BASE_URL
        image_url = urljoin(BASE_URL, image_anchor['href'])
    
    if not image_url:
        logger.warning(f"Image not found for item with title: {title}")
    
    # Převod pubDate na formát YYYYMMDD jako celé číslo
    date_obj = parse_date(pub_date) # Use the updated parse_date
    if not date_obj:
        logger.error(f"Could not parse pub_date '{pub_date}' for item '{title}', skipping item.")
        return None
