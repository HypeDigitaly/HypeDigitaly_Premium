...
SITEMAP_URL = "https://healthytwenty.cz/sitemap_index.xml"  # URL of the XML sitemap
...
- Each question must include:
  1. Primary question formulation
  2. Alternative question formulation
  3. Keyword-focused formulation
  4. 2-3 most relevant keywords/key phrases, MUST include specific names when present:
     * Department names (e.g., "Cardiology Department")
     * Staff names (e.g., "[First Name Last Name]")
     * Location names (e.g., "[City / Location name]")
Format: "Primary Question? | Alternative Question? | Keyword Question? | keyword1 | keyword2 | keyword3"

# SPECIAL HANDLING FOR TEAM/STAFF CONTENT:
- For questions about team members or staff, ALWAYS include ALL available images of that person in the answer
- Format: Include every image of the person using markdown syntax: ![Name - Position/Role](image_url)
- Multiple images should be placed at the end of the answer text
- Include both individual photos and group photos where the team member appears
- For team overview questions, include ALL team member images

# SPECIAL HANDLING FOR DEPARTMENT CONTENT:
- For questions about departments or facilities, ALWAYS include ALL related images
- Format: Include every department image using markdown syntax: ![Department Area - Description](image_url)
- Multiple images should be organized from general to specific views
- For department overview questions, include ALL relevant facility images

# EXPECTED JSON FORMAT (SINGLE COMPLETE RESPONSE):
{{
  "qa_pairs": [
    {{
      "Question": "Primary Question? | Alternative Question? | Keyword Question? | specific_name | keyword2 | keyword3",
      "Answer": "Detailed answer including all relevant facts and data points. [Relevant link text](URL) when applicable. ![Image description](image_url) when directly relevant to the answer. For departments/team members: include ALL related photos."
    }},
    ...
  ]
}}
...
2. QUESTION STRUCTURE:
   - Primary question (natural, conversational)
   - Alternative formulation (different perspective)
   - Keyword-focused version (search-friendly)
   - 2-3 most relevant keywords/phrases (pipe-separated)
   - ALWAYS include specific names (department, staff, location) as one of the keywords when present
3. QUANTITY: Generate 10-30+ Q&A pairs until ALL relevant content is captured
4. GRANULARITY: Break complex information into multiple specific Q&A pairs
5. COVERAGE - Create questions for:
   - Core facts and data
   ...
   - Contact information
   - Unique features
   - Team members (with ALL their images)
   - Department facilities and equipment (with ALL related images)
6. Each question requires 3 formulations (separated by |)
7. LINK AND IMAGE INTEGRATION:
   ...
   - For team members/staff: Include ALL their available images
   - For departments: Include ALL facility and equipment images
8. Focus on relevant content only
9. Maintain valid JSON structure
10. Keep answers as single-level text with integrated links/images
