...
def parse_contacts(html_content):
    ...
    contacts = []
    for contact_div in soup.find_all('div', class_='osoba'):
...
def initialize_json_files():
    files_to_initialize = {
        ...
        "payloads/all_documents.json": {
            "schema": {
                "searchableFields": ["Name", "DocumentsCategory", "URL"],
                "metadataFields": ["DocumentsCategory", "Category"]
            },
            "name": "all_documents"
        }
    }
    ...
    for file_path, structure in files_to_initialize.items():
        category = file_path.split('_')[-1].split('.')[0].capitalize()
        initial_data = {
            "data": {
...
def update_documents_file(new_documents):
    ...
    with open(filename, 'r+', encoding='utf-8') as f:
        ...
        for document in new_documents:
            document["DocumentsCategory"] = document.pop("Category", "")
...
def save_payload_to_file(url, content, section, metadata):
    ...
    for item in payload["data"]["items"]:
        ...
        for key in schema["searchableFields"] + schema["metadataFields"]:
            if key not in item:
