...
RSS_URL = "https://www.litomerice.cz/dokumenty-meu?format=feed&type=rss"
...
SCRIPT_NAME = "scrape_dokumenty_mestskeho_uradu"
...
def parse_rss_feed(content):
    ...
    sorted_items = sorted(
        items,
        key=lambda x: parse_date(x.pubDate.text.strip()),
        reverse=True
    )
...
def extract_item_data(item):
    ...
    categories = ["Administrativa_Uredni_Zalezitosti", "Dokumenty_Mestskeho_Uradu"]
...
def create_initial_payload(filename_without_extension):
    return {
        "data": {
            ...
            "name": f"table_dokumenty_mestskeho_uradu_{filename_without_extension}",
            "items": []
        }
    }
...
def parse_litomerice_news():
    ...
    for entry in feed.entries:
        ...
        news_item = {
            ...
            'date': int(pub_date.strftime('%Y%m%d'))
        }
        
        news_items.append(news_item)
    
    logger.info(f"Found {len(news_items)} items to process")
    return news_items
...
def main():
    try:
        ...
        # Save and upload files - one per year
        for year, payload in payloads_by_year.items():
            if payload['data']['items']:
                output_file = f"payloads/table_dokumenty_mestskeho_uradu_{year}.json"
        ...
        logger.info(f"Processed total {total_items} items, {processed_items} successfully processed.")
