...
# Setup logging
SCRIPT_NAME = "scrape_contacts_rada"
...
SINGLE_URL = "https://www.litomerice.cz/zakladni-informace-rm"
...
def extract_contacts(soup):
    items = []
    counter = 1  # Initialize counter for RAD-[number]
    
    members_table = soup.find('table', class_='table table-striped table-bordered')
    if not members_table:
        logger.warning("Could not find the Rada members table")
        return items
        
    for row in members_table.find_all('tr'):
        cols = row.find_all('td')
        if len(cols) == 3 and cols[0].text.strip() and cols[2].find('a'):
            name = cols[0].text.strip()
            original_department = cols[1].text.strip()  # Store original department value
            email = cols[2].find('a')['href'].replace('mailto:', '')
            ...
            if name and email and '@litomerice.cz' in email:
                ...
                items.append({
                    ...
                    'Title': title,
                    'Role': 'Člen',
                    'Department': f'RAD-{counter}',  # New department format
                    'Subdepartment': original_department,  # Move original department here
                    'PhoneNumber': None,
                    'Email': email,
                    'Origin': 'Rada',
                ...
                counter += 1  # Increment counter for next item
...
def process_single_url():
    ...
    try:
        ...
        if html_content:
            ...
            if items:
                file_path = os.path.join(OUTPUT_DIRECTORY, "contacts_rada.json")
                json_output = {
                    "data": {
                        "schema": {
                ...
                # Upload to Voiceflow
                upload_to_voiceflow("contacts_rada", items)
