...
RSS_URL = "https://www.mkz-ltm.cz/xml/kamvltm.php"  # Updated RSS feed URL
...
PROCESS_MAX_DATE = "20251231" 
##PROCESS_MAX_DATE = None  # You can set this to None or remove this line entirely
##PROCESS_MAX_DATE = (datetime.now() + timedelta(days=7)).strftime('%Y%m%d') if PROCESS_MAX_DATE is None else PROCESS_MAX_DATE
dateThreshold = "20250101"  # Lower date threshold for processing (YYYYMMDD)
...
def parse_rss_feed(content):
    ...
    filtered_items = []
    for item in items:
        # Use 'date' tag instead of 'pubDate'
        date_str = item.find('date').text.strip()
        
        # Convert DD.MM.YYYY HH:MM format to datetime
        try:
            date_parts = date_str.split(' ')[0].split('.')
            time_parts = date_str.split(' ')[1].split(':')
            pub_date_dt = datetime(
    ...
    filtered_items.sort(
        key=lambda x: datetime.strptime(x.find('date').text.strip().split(' ')[0], '%d.%m.%Y'),
        reverse=True
    )
    
    # Save the newest processed date
    if filtered_items:
        newest_date_str = filtered_items[0].find('date').text.strip()
        save_last_processed_date(newest_date_str)
    
    return filtered_items, last_processed_date

def extract_item_data(item):
    """Extract data from XML item"""
    title = item.find('title').get_text(strip=True).replace('![CDATA[', '').replace(']]', '')
    link = item.find('link').get_text(strip=True).replace('![CDATA[', '').replace(']]', '')
    
    # Get description and clean CDATA tags
    description = item.find('description').get_text(strip=True).replace('![CDATA[', '').replace(']]', '')
    
    # Get image URL directly from photo tag
    image_url = item.find('photo').get_text(strip=True)
    if not image_url.startswith('http'):
        image_url = BASE_URL + image_url
    
    # Parse date from date tag
    date_str = item.find('date').text.strip()
    try:
        # Convert DD.MM.YYYY HH:MM format to YYYYMMDD
        date_parts = date_str.split(' ')[0].split('.')
        formatted_date = int(f"{date_parts[2]}{date_parts[1]}{date_parts[0]}")
    except (IndexError, ValueError) as e:
    ...
    # Replace the existing categorization logic with fixed categories
...
def create_payloads_structure():
    ...
    for year in years:
        payloads[year] = create_initial_payload(f"table_events_{year}")
...
def main():
    try:
        ...
        # Fix: Get newest date from the first item's date tag and format it correctly
        newest_date = rss_items[0].find('date').text.strip()
        # Convert DD.MM.YYYY HH:MM to YYYYMMDD
        date_parts = newest_date.split(' ')[0].split('.')
        newest_date_formatted = f"{date_parts[2]}{date_parts[1]}{date_parts[0]}"
        
        # Fix: Handle last processed date formatting
        if last_processed_date:
            date_parts = last_processed_date.split(' ')[0].split('.')
            last_processed_formatted = f"{date_parts[2]}{date_parts[1]}{date_parts[0]}"
        else:
            ...
            if item_data:
                ...
                if year not in payloads_by_year:
                    if year == current_year:
                        filename_without_ext = f"table_events_{year}_{last_processed_formatted}_{newest_date_formatted}"
                    else:
                        filename_without_ext = f"table_events_{year}"
