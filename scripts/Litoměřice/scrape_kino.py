...
BASE_URL = "https://kinoostrov.cz"
RSS_URL = "https://kinoostrov.cz/feed/"
...
def parse_date(date_str):
    """Parse date from WordPress RSS format"""
    try:
        # Parse WordPress format (e.g. 'Tue, 17 Dec 2024 07:44:59 +0000')
...
def parse_rss_feed(content):
    ...
    items = feed.entries  # WordPress RSS feed uses 'item' instead of 'entry'
    
    logger.info(f"Found {len(items)} total items in RSS feed")
    
    filtered_items = []
    for item in items:
        pub_date = item.published  # WordPress uses 'pubDate'
        try:
            pub_date_dt = datetime.strptime(pub_date, '%a, %d %b %Y %H:%M:%S %z')
        except Exception as e:
            logger.error(f"Error parsing date '{pub_date}': {str(e)}")
            continue
        ...
        if date_int >= int(dateThreshold):
            ...
            logger.debug(f"Skipping item with date {pub_date_dt.strftime('%Y-%m-%d')} - before threshold")
    ...
    if filtered_items:
        newest_date = filtered_items[0].published
        save_last_processed_date(newest_date)
    
    return filtered_items, last_processed_date

def extract_item_data(item):
    """Extract data from Kinoostrov RSS feed entry"""
    title = item.title
    link = item.link
    
    # Parse content:encoded for full description
    if hasattr(item, 'content'):
        description = item.content[0].value
    else:
        description = item.description
    
    # Clean up description - remove WordPress footer
    description = description.split('The post')[0].strip()
    # Remove HTML tags
    ...
    if formatted_date < int(dateThreshold):
        logger.info(f"Skipping movie with date {formatted_date} (before threshold {dateThreshold})")
        return None
    
    logger.info(f"Processing movie with URL: {link}")
    
    # Always include these two categories
    categories = ["Media_Komunikace", "Kultura_Pamatkova_Pece"]
    
    # If categorization is enabled, add additional categories from Claude
    if ENABLE_CATEGORIZATION:
        claude_categories = get_categories_from_claude(title, description)
        categories.extend(claude_categories)
...
def create_initial_payload(filename_without_extension):
    return {
        "data": {
            "schema": {
                "searchableFields": ["Title", "URL", "Description", "Date"],
                "metadataFields": ["Category", "Date"]
            },
            "name": f"table_kino_{filename_without_extension}",  # Add kino prefix
            "items": []
        }
    }
...
Nepoužívej kategorii "Kultura_Pamatkova_Pece" - ta bude přidána automaticky.

Příklad odpovědi: ["Rozvoj_Projekty"]
...
ODPOVĚZ POUZE JSON POLEM KATEGORIÍ, NIC JINÉHO."""
    ...
    try:
        ...
        # Add "Kultura_Pamatkova_Pece" to the beginning of the categories list
        categories.insert(0, "Kultura_Pamatkova_Pece")  # Changed from Media_Komunikace
        logger.info(f"Claude categories for '{title}': {categories}")
        return categories
    except Exception as e:
        logger.error(f"Error getting categories from Claude: {str(e)}")
        return ["Kultura_Pamatkova_Pece"]  # Changed from Media_Komunikace
...
def main():
    try:
        ...
        filtered_items, last_processed_date = parse_rss_feed(rss_content)
        ...
        # Process items and organize by year
        ...
        logger.info(f"Processed total {len(filtered_items)} new items")
